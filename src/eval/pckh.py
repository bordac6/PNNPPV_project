...
def run_pckh(model_name, predfile):
    ...
    nsamples = annot['joint_uvd'].shape[1]
    train_val_treshold = nsamples * 0.8

    hand_points = [0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 35]
    gt_points = annot['joint_uvd'][0, train_val_treshold:, hand_points, :]
...
#     PCKh = np.divide(100. * np.sum(less_than_threshold, axis=1), jnt_count)
    uv_error = preds - gt_points
    uv_error = np.linalg.norm(uv_error, axis=1)
    print(uv_error)
    print('shape: {}'.format(uv_error.shape))
