...
def bottleneck_block(bottom, num_out_channels, block_name):
    ...
    _x = Conv2D(round(num_out_channels / 2), kernel_size=(1, 1), activation='relu', padding='same',
                name=block_name + '_conv_1x1_x1')(bottom)
    _x = BatchNormalization()(_x)
    _x = Conv2D(round(num_out_channels / 2), kernel_size=(3, 3), activation='relu', padding='same',
...
def bottleneck_mobile(bottom, num_out_channels, block_name):
    ...
    # residual: 3 conv blocks,  [num_out_channels/2  -> num_out_channels/2 -> num_out_channels]
    _x = SeparableConv2D(round(num_out_channels / 2), kernel_size=(1, 1), activation='relu', padding='same',
                         name=block_name + '_conv_1x1_x1')(bottom)
    _x = BatchNormalization()(_x)
    _x = SeparableConv2D(round(num_out_channels / 2), kernel_size=(3, 3), activation='relu', padding='same',
                         name=block_name + '_conv_3x3_x2')(_x)
    _x = BatchNormalization()(_x)
